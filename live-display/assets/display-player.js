...
function renderRecent() {
  ...
  function renderEventTimer() {
    ...
    return '<article class="display-slide" data-slide="event_timer">'
      ...
      + '<section class="event-timer-panel event-timer-main"><span>Time left to dance</span><strong data-live-countdown="' + esc(eventTarget) + '">' + esc(eventTimer) + '</strong><em>until the planned finish</em></section>'
      + '<section class="event-timer-panel"><span>Requests close</span><strong' + (requestsOpen && requestTarget ? ' data-live-countdown="' + esc(requestTarget) + '"' : '') + '>' + esc(requestsOpen ? requestTimer : 'Closed') + '</strong><em>' + esc(requestsOpen ? 'left to send requests' : 'Thanks for the requests tonight') + '</em></section>'
      + '</div>'
      + '</div></article>';
  }
