    ...
    if (!dttd_spotify_table_exists('player_nodes') || !dttd_spotify_table_exists('node_commands')) {
        ...
        try {
            ...
            if ($device && !empty($device['id'])) {
                ...
                dttd_spotify_prepare_player_device((string)$device['id'], $deck, $prepareTestTrack, $preparePlayTest);

                $nodeFlash = 'Prepared ' . dttd_spotify_node_label($node) . ' for Deck ' . strtoupper($deck) . ' using Spotify device "' . (string)($device['name'] ?? 'device') . '".';
                if ($preparePlayTest && $prepareTestTrack !== '') {
                    $nodeFlash .= ' Test track was briefly played and paused.';
                }
            } else {
                $nodeFlash = 'Wake command queued for ' . dttd_spotify_node_label($node) . ' and assigned to Deck ' . strtoupper($deck) . '. Wait 10–20 seconds, then press Prepare again to bind the Spotify device when it appears.';
            }
        } catch (Throwable $e) {
