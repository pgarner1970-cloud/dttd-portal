...
function dttd_spotify_command_label($command) {
    $labels = [
        ...
        'display_lite' => 'Display Lite Mode',
        'display_full' => 'Display Full Mode',
        'display_blank' => 'Blank HDMI Screen',
        'display_wake' => 'Wake HDMI Screen',
        'display_status' => 'Refresh Display Status',
    ];
    ...
    if (!dttd_spotify_table_exists('player_nodes') || !dttd_spotify_table_exists('node_commands')) {
        ...
        try {
            ...
            if (!$node) {
                ...
                if ($command === 'display_lite') {
                    $payload = json_encode(['mode' => 'lite']);
                }

                if ($command === 'display_full') {
                    $payload = json_encode(['mode' => 'full']);
                }
              ...
              <?php if ($hasNode): ?>
                ...
                <div class="pi-control-block">
                  ...
                  <form class="pi-display-actions" method="post">
                    ...
                    <button class="apply" type="submit" name="command" value="display_start">Start / Apply</button>
                    <button type="submit" name="command" value="display_restart">Restart Display</button>
                    <button class="blank" type="submit" name="command" value="display_blank">Blank Screen</button>
                    <button class="wake" type="submit" name="command" value="display_wake">Wake Screen</button>
                  </form>
                  <p class="pi-display-hint"><strong>Start / Apply</strong> starts the HDMI player in the selected mode. <strong>Restart Display</strong> restarts the browser. <strong>Blank Screen</strong> should blank HDMI output; <strong>Wake Screen</strong> restores it. Spotify/local music playback is not affected.</p>
