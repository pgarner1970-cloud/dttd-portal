...
function dttd_spotify_prepare_player_device($deviceId, $deck, $testTrackId = '', $playTest = false) {
    ...
    // Transfer without playing first. This is quiet and often enough to make the device current.
    dttd_spotify_prepare_put('https://api.spotify.com/v1/me/player', json_encode([
        'device_ids' => [$deviceId],
        'play' => false,
    ]), $deck);

    if ($playTest && $testTrackId !== '') {
        usleep(350000);
        ...
        usleep(1800000);
        try {
            dttd_spotify_prepare_put('https://api.spotify.com/v1/me/player/pause?device_id=' . rawurlencode($deviceId), '', $deck);
...
function dttd_spotify_deck_token_payload($deck, $node) {
    $deck = strtolower((string)$deck) === 'b' ? 'b' : 'a';
    $token = dttd_spotify_user_access_token_for_deck($deck);

    if ($token === '') {
        throw new RuntimeException('No Spotify access token was available for Deck ' . strtoupper($deck) . '.');
    }
    ...
    if ($profile) {
        $profileLabel = (string)($profile['label'] ?? $profile['name'] ?? ('Deck ' . strtoupper($deck) . ' profile'));
        $expiresAt = (string)($profile['expires_at'] ?? '');
    } else {
        $profileLabel = 'Primary / legacy Spotify account';
        $legacyExpires = (int)dttd_spotify_tool_setting('spotify_token_expires_at', '0');
        if ($legacyExpires > 0) {
            $expiresAt = date('Y-m-d H:i:s', $legacyExpires);
        }
    }

    $spotifyName = trim((string)($node['spotify_name'] ?? ''));
    if ($spotifyName === '') {
        $spotifyName = trim((string)($node['display_name'] ?? ''));
    }
    if ($spotifyName === '') {
        $spotifyName = 'DMX Deck ' . strtoupper($deck);
    }

    return [
        'deck' => strtoupper($deck),
        'spotify_name' => $spotifyName,
        'account_label' => $profileLabel,
        'access_token' => $token,
        'expires_at' => $expiresAt,
        'generated_at' => date('c'),
        'instructions' => [
            'mode' => 'librespot_access_token',
            'restart_raspotify' => true,
            'write_env' => [
                'LIBRESPOT_NAME' => $spotifyName,
                'LIBRESPOT_ACCESS_TOKEN' => $token,
                'LIBRESPOT_DEVICE_TYPE' => 'speaker',
                'LIBRESPOT_DISCOVERY_BACKEND' => 'avahi',
            ],
        ],
    ];
}
...
$preparePlayTest = dttd_spotify_tool_setting('spotify_prepare_play_test', '0') === '1';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['node_action'] ?? '') === 'save_prepare_settings') {
    ...
    $preparePlayTest = !empty($_POST['prepare_play_test']);
    try {
        dttd_spotify_tool_set('spotify_prepare_test_track_id', $prepareTestTrack);
        dttd_spotify_tool_set('spotify_prepare_play_test', $preparePlayTest ? '1' : '0');
        $nodeFlash = 'Prepare Player settings saved.';
    } catch (Throwable $e) {
        $nodeError = 'Could not save Prepare Player settings: ' . $e->getMessage();
    }
}

if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['node_action'] ?? '') === 'configure_spotify_account') {
    ...
    if (!dttd_spotify_table_exists('player_nodes') || !dttd_spotify_table_exists('node_commands')) {
        ...
        try {
            ...
            $payload = dttd_spotify_deck_token_payload($deck, $node);

            db()->prepare("
                INSERT INTO node_commands (node_key, command, payload, status)
                VALUES (?, 'configure_spotify_account', ?, 'pending')
            ")->execute([$nodeKey, json_encode($payload)]);

            $nodeFlash = 'Spotify account token/config command queued for ' . dttd_spotify_node_label($node) . ' on Deck ' . strtoupper($deck) . '. The Pi agent must support configure_spotify_account for this to apply.';
        } catch (Throwable $e) {
            $nodeError = 'Could not queue Spotify account config: ' . $e->getMessage();
        }
    }
}

if ($_SERVER['REQUEST_METHOD'] === 'POST' && in_array(($_POST['node_action'] ?? ''), ['prepare_player', 'force_test_player'], true)) {
    $nodeKey = trim((string)($_POST['node_key'] ?? ''));
    $deck = strtolower((string)($_POST['deck'] ?? 'a')) === 'b' ? 'b' : 'a';
    $forceTest = ($_POST['node_action'] ?? '') === 'force_test_player';

    if (!dttd_spotify_table_exists('player_nodes') || !dttd_spotify_table_exists('node_commands')) {
        $nodeError = 'Player node tables are not available yet.';
    } elseif ($nodeKey === '') {
        $nodeError = 'No player node was selected.';
    } elseif ($forceTest && $prepareTestTrack === '') {
        $nodeError = 'Set a Prepare Player test track before using Force Test.';
    } else {
        try {
            $stmt = db()->prepare("SELECT * FROM player_nodes WHERE node_key = ? LIMIT 1");
            $stmt->execute([$nodeKey]);
            ...
            ")->execute([$nodeKey, json_encode([
                'reason' => $forceTest ? 'force_test_player' : 'prepare_player',
                'deck' => strtoupper($deck),
            ...
            if (dttd_spotify_queue_connected_for_deck($deck)) {
                ...
                if (!$device) {
                    [$device, $lastDevices] = dttd_spotify_wait_for_node_device($node, $deck, $forceTest ? 24 : 12);
                }
            }
            ...
            if ($device && !empty($device['id'])) {
                ...
                $shouldPlayTest = $forceTest || $preparePlayTest;
                dttd_spotify_prepare_player_device((string)$device['id'], $deck, $prepareTestTrack, $shouldPlayTest);

                $nodeFlash = ($forceTest ? 'Force tested ' : 'Prepared ') . dttd_spotify_node_label($node) . ' for Deck ' . strtoupper($deck) . ' using Spotify device "' . (string)($device['name'] ?? 'device') . '".';
                if ($shouldPlayTest && $prepareTestTrack !== '') {
                    $nodeFlash .= ' Test track was sent, briefly played and paused.';
                } elseif ($prepareTestTrack === '') {
                    $nodeFlash .= ' No test track is configured, so playback was not forced.';
                }
            } else {
            ...
            $nodeError = ($forceTest ? 'Force Test failed: ' : 'Prepare Player failed: ') . $e->getMessage();
        }
    }
}
    ...
    <section class="pi-node-panel">
      ...
      <form class="pi-prepare-settings" method="post">
        ...
        <label class="checkline">
          <input type="checkbox" name="prepare_play_test" value="1" <?= $preparePlayTest ? 'checked' : '' ?>>
          Briefly play then pause test track
        </label>
          ...
          <?php foreach ($playerNodes as $node): ?>
            ...
            <article class="pi-node-card">
              ...
              <form class="pi-prepare-actions" method="post">
                ...
                <button type="submit" name="deck" value="a">Prepare Deck A</button>
                <button type="submit" name="deck" value="b">Prepare Deck B</button>
              </form>

              <form class="pi-prepare-actions pi-account-actions" method="post">
                <input type="hidden" name="node_action" value="configure_spotify_account">
                <input type="hidden" name="node_key" value="<?= h($node['node_key'] ?? '') ?>">
                <button type="submit" name="deck" value="a" onclick="return confirm('Send Deck A Spotify access token/config to <?= h(dttd_spotify_node_label($node)) ?>?')">Send Account A</button>
                <button type="submit" name="deck" value="b" onclick="return confirm('Send Deck B Spotify access token/config to <?= h(dttd_spotify_node_label($node)) ?>?')">Send Account B</button>
              </form>

              <form class="pi-prepare-actions pi-force-actions" method="post">
                <input type="hidden" name="node_action" value="force_test_player">
                <input type="hidden" name="node_key" value="<?= h($node['node_key'] ?? '') ?>">
                <button type="submit" name="deck" value="a">Force Test A</button>
                <button type="submit" name="deck" value="b">Force Test B</button>
              </form>
