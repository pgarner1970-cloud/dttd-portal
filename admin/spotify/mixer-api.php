...
function mx_set($key, $value) {
    mx_debug_trace('db_setting_write', ['key' => (string)$key, 'value_summary' => is_string($value) && strlen($value) > 220 ? substr($value, 0, 220) . '…' : $value]);
...
function mx_debug_enabled() {
    ...
    $setting = strtolower(trim((string)mx_setting('spotify_mixer_debug_enabled', '0')));
    $enabled = in_array($setting, ['1', 'true', 'yes', 'on'], true);
    return $enabled;
}

function mx_debug_request_id() {
    if (empty($GLOBALS['mx_debug_request_id'])) {
        try { $GLOBALS['mx_debug_request_id'] = bin2hex(random_bytes(6)); }
        catch (Throwable $e) { $GLOBALS['mx_debug_request_id'] = substr(md5(uniqid('', true)), 0, 12); }
    }
    return $GLOBALS['mx_debug_request_id'];
}

function mx_debug_started_at() {
    if (empty($GLOBALS['mx_debug_started_at'])) $GLOBALS['mx_debug_started_at'] = microtime(true);
    return (float)$GLOBALS['mx_debug_started_at'];
}

function mx_debug_sanitize($value, $depth = 0) {
    if ($depth > 5) return '[max-depth]';
    if (is_array($value)) {
        $out = [];
        foreach ($value as $k => $v) {
            $key = is_string($k) ? strtolower($k) : (string)$k;
            if (preg_match('/token|secret|password|cookie|authorization|client_secret|refresh/i', $key)) {
                $out[$k] = '[redacted]';
            } else {
                $out[$k] = mx_debug_sanitize($v, $depth + 1);
            }
        }
        return $out;
    }
    if (is_object($value)) return mx_debug_sanitize((array)$value, $depth + 1);
    if (is_string($value)) {
        $value = preg_replace('/Bearer\s+[A-Za-z0-9._~+\/=:-]+/i', 'Bearer [redacted]', $value);
        $value = preg_replace('/access_token=[^&\s]+/i', 'access_token=[redacted]', $value);
        $value = preg_replace('/refresh_token=[^&\s]+/i', 'refresh_token=[redacted]', $value);
        if (strlen($value) > 1200) return substr($value, 0, 1200) . '…[truncated]';
    }
    return $value;
}

function mx_debug_track_summary($track) {
    ...
    return [
        ...
        'position_base_ms' => isset($track['position_base_ms']) ? (int)$track['position_base_ms'] : null,
        'paused_position_ms' => isset($track['paused_position_ms']) ? (int)$track['paused_position_ms'] : null,
...
function mx_debug_playback_summary($pb) {
    if (!is_array($pb)) return null;
    $item = $pb['item'] ?? [];
    $artists = [];
    foreach (($item['artists'] ?? []) as $artist) if (!empty($artist['name'])) $artists[] = $artist['name'];
    return [
        'device_id' => (string)($pb['device']['id'] ?? ''),
        'device_name' => (string)($pb['device']['name'] ?? ''),
        'is_playing' => !empty($pb['is_playing']),
        'progress_ms' => isset($pb['progress_ms']) ? (int)$pb['progress_ms'] : null,
        'track_id' => (string)($item['id'] ?? ''),
        'track_title' => (string)($item['name'] ?? ''),
        'track_artist' => implode(', ', $artists),
        'duration_ms' => isset($item['duration_ms']) ? (int)$item['duration_ms'] : null,
    ];
}

function mx_debug_account_summary($deck = null) {
    $deck = strtolower((string)$deck);
    if ($deck !== 'a' && $deck !== 'b') return ['deck' => $deck, 'profile' => 'primary_or_unspecified'];
    try {
        if (function_exists('dttd_spotify_profile_summary_for_deck')) {
            $summary = dttd_spotify_profile_summary_for_deck($deck);
            return [
                'deck' => strtoupper($deck),
                'assigned' => !empty($summary['assigned']),
                'connected' => !empty($summary['connected']),
                'profile_id' => (int)($summary['id'] ?? 0),
                'slot' => (int)($summary['slot'] ?? 0),
                'label' => (string)($summary['label'] ?? ''),
                'display' => (string)($summary['display'] ?? ''),
                'email' => (string)($summary['email'] ?? ''),
                'warning' => (string)($summary['warning'] ?? ''),
            ];
        }
    } catch (Throwable $e) {
        return ['deck' => strtoupper($deck), 'error' => $e->getMessage()];
    }
    return ['deck' => strtoupper($deck), 'profile' => 'summary_unavailable'];
}

function mx_debug_device_summary_list($devices) {
    $out = [];
    foreach ((array)$devices as $d) {
        ...
        $out[] = [
            ...
            'is_restricted' => !empty($d['is_restricted']),
            'volume_percent' => isset($d['volume_percent']) ? (int)$d['volume_percent'] : null,
        ];
    }
    return $out;
}

function mx_debug_spotify_error_detail($response, $curl_error = '') {
    $detail = ['curl_error' => (string)$curl_error, 'raw' => is_string($response) ? $response : ''];
    ...
    if (is_array($decoded)) {
        ...
        if (isset($decoded['error'])) {
            if (is_array($decoded['error'])) {
                ...
                $detail['spotify_error'] = $decoded['error'];
            }
        }
    }
    return mx_debug_sanitize($detail);
}

function mx_debug_fetch_devices_for_deck($deck, $context = '') {
    if (!mx_debug_enabled()) return;
    $deck = strtolower((string)$deck);
    ...
    try {
        ...
        mx_debug_trace('spotify_devices_diagnostic', [
            'context' => $context,
            'deck' => $deck,
            'account' => mx_debug_account_summary($deck),
            'http_status' => $status,
            'curl_error' => $error,
            'device_count' => count($decoded['devices'] ?? []),
            'devices' => mx_debug_device_summary_list($decoded['devices'] ?? []),
            'error_detail' => ($status < 200 || $status >= 300 || $response === false) ? mx_debug_spotify_error_detail($response, $error) : null,
        ]);
    } catch (Throwable $e) {
        mx_debug_trace('spotify_devices_diagnostic_error', [
            'context' => $context,
            'deck' => $deck,
            'account' => mx_debug_account_summary($deck),
            'error' => $e->getMessage(),
...
function mx_debug_trace($event, array $data = []) {
    if (!mx_debug_enabled()) return;
    if (!isset($GLOBALS['mx_debug_trace']) || !is_array($GLOBALS['mx_debug_trace'])) $GLOBALS['mx_debug_trace'] = [];
    $GLOBALS['mx_debug_trace'][] = [
        't' => date('c'),
        'elapsed_ms' => (int)round((microtime(true) - mx_debug_started_at()) * 1000),
        'source' => 'backend',
        'event' => (string)$event,
        'data' => mx_debug_sanitize($data),
    ];
    if (count($GLOBALS['mx_debug_trace']) > 500) array_shift($GLOBALS['mx_debug_trace']);
}

function mx_debug_log_dir() {
    $dir = __DIR__ . '/debug-logs';
    if (!is_dir($dir)) @mkdir($dir, 0755, true);
    if (is_dir($dir) && is_writable($dir)) return $dir;
    return sys_get_temp_dir();
}

function mx_debug_log_file() {
    return rtrim(mx_debug_log_dir(), '/\\') . '/mixer-debug-' . date('Ymd') . '.jsonl';
}

function mx_debug_log_meta() {
    $file = mx_debug_log_file();
    return [
        'enabled' => mx_debug_enabled(),
        'file' => basename($file),
        'size_bytes' => is_file($file) ? (int)filesize($file) : 0,
        'updated_at' => is_file($file) ? date('Y-m-d H:i:s', (int)filemtime($file)) : '',
    ];
}

function mx_debug_flush($ok = true, array $extra = []) {
    if (!mx_debug_enabled()) return;
    $events = $GLOBALS['mx_debug_trace'] ?? [];
    if (!$events && empty($extra)) return;
    $entry = [
        'request_id' => mx_debug_request_id(),
        't' => date('c'),
        'elapsed_ms' => (int)round((microtime(true) - mx_debug_started_at()) * 1000),
        'ok' => (bool)$ok,
        'method' => $_SERVER['REQUEST_METHOD'] ?? '',
        'action' => (string)($_POST['action'] ?? $_GET['action'] ?? 'state'),
        'deck' => (string)($_POST['deck'] ?? $_GET['deck'] ?? ''),
        'events' => $events,
        'extra' => mx_debug_sanitize($extra),
    ];
    $line = json_encode($entry, JSON_UNESCAPED_SLASHES) . PHP_EOL;
    @file_put_contents(mx_debug_log_file(), $line, FILE_APPEND | LOCK_EX);
    $GLOBALS['mx_debug_trace'] = [];
}

function mx_debug_clear_log() {
    foreach (glob(rtrim(mx_debug_log_dir(), '/\\') . '/mixer-debug-*.jsonl') ?: [] as $file) {
        @unlink($file);
    }
}

function mx_debug_download_log() {
    $file = mx_debug_log_file();
    if (!is_file($file)) {
        header('Content-Type: text/plain; charset=utf-8');
        echo "No mixer debug log found for today.\n";
        exit;
    }
...
function mx_spotify_user_get($url) {
    ...
    mx_debug_trace('spotify_http_get', ['url' => $url]);
    try {
        $result = dttd_spotify_http_get($url, [
        'Authorization: Bearer ' . $token,
        'Accept: application/json',
        ]);
        mx_debug_trace('spotify_http_get_ok', ['url' => $url, 'keys' => is_array($result) ? array_slice(array_keys($result), 0, 12) : []]);
        return $result;
    } catch (Throwable $e) {
        mx_debug_trace('spotify_http_get_error', ['url' => $url, 'error' => $e->getMessage()]);
        throw $e;
    }
}
...
function mx_spotify_put($url, $body = '', $deck = null) {
    $deckNorm = strtolower((string)$deck);
    $token = ($deckNorm === 'a' || $deckNorm === 'b') ? dttd_spotify_user_access_token_for_deck($deckNorm) : dttd_spotify_user_access_token();
    $payload = $body !== '' ? json_decode((string)$body, true) : '';
    mx_debug_trace('spotify_http_put', [
        'deck' => $deck,
        'account' => mx_debug_account_summary($deck),
        'url' => $url,
        'body' => $payload,
    ...
    if (!function_exists('curl_init')) {
        mx_debug_trace('spotify_http_put_error', ['deck' => $deck, 'url' => $url, 'error' => 'PHP cURL is not available.']);
    ...
    mx_debug_trace($ok ? 'spotify_http_put_ok' : 'spotify_http_put_error', [
        'deck' => $deck,
        'account' => mx_debug_account_summary($deck),
        'url' => $url,
        'http_status' => $status,
        ...
        'response_text' => (!is_array($decoded) && is_string($response)) ? $response : '',
    ]);

    if (!$ok) {
        if ($deckNorm === 'a' || $deckNorm === 'b') mx_debug_fetch_devices_for_deck($deckNorm, 'after_put_failure');
        $detail = mx_debug_spotify_error_detail($response, $error);
        $message = 'Spotify playback request failed';
        if ($status > 0) $message .= ' (HTTP ' . $status . ')';
        if (!empty($detail['spotify_error']['message'])) $message .= ': ' . $detail['spotify_error']['message'];
        elseif ($error !== '') $message .= ': ' . $error;
        else $message .= '.';
...
function mx_play_track($device_id, $track_id, $position_ms = null, $deck = null) {
    mx_debug_trace('play_track_start', [
        'deck' => $deck,
        'account' => mx_debug_account_summary($deck),
        'device_id' => $device_id,
        'track_id' => $track_id,
    ...
    mx_debug_fetch_devices_for_deck($deck, 'before_play_track');
    $device_id = trim((string)$device_id);
    ...
    mx_debug_trace('play_track_end', ['deck' => $deck, 'device_id' => $device_id, 'track_id' => $track_id, 'position_ms' => $position]);
}
...
function mx_store_loaded_track($deck, $track) {
    $deck = $deck === 'b' ? 'b' : 'a';
    mx_debug_trace('store_loaded_track', ['deck' => $deck, 'track' => mx_debug_track_summary(is_array($track) ? $track : [])]);
...
function mx_reset_runtime_track_state($track) {
    $clean = mx_clean_track(is_array($track) ? $track : []);
    ...
    $clean['playback_started_at'] = null;
    $clean['expected_finish_at'] = null;
    $clean['history_logged_at'] = null;
    ...
    $clean['local_playback_mode'] = '';
    $clean['local_autoplay_after_prepare'] = false;
    $clean['local_autoplay_requested_at'] = null;
    return $clean;
}
function mx_pause_deck_output_for_unload($deck, $track = null) {
    $deck = $deck === 'b' ? 'b' : 'a';
    $track = is_array($track) ? $track : mx_json('spotify_mixer_loaded_' . $deck, []);
    if (mx_is_local_track($track)) {
        try { mx_queue_deck_node_command($deck, 'local_pause', []); } catch (Throwable $ignoredLocalPause) {}
        if (is_array($track) && !empty($track['id'])) {
            try { mx_local_pause_track($deck, $track); } catch (Throwable $ignoredLocalState) {}
        }
        return;
    }
    $device = mx_deck_device_id($deck);
    if ($device !== '') {
        try { mx_pause($device, $deck); } catch (Throwable $ignoredSpotifyPause) {}
    }
}
...
function mx_arm_loaded_track_for_playback($deck, &$track, $start_position_ms = 0) {
    ...
    unset($track['pause_hold_until'], $track['paused_by_mixer_at']);
...
function mx_sync_loaded_position_from_playback($deck, $loaded, $device_id, $playback = null) {
    ...
    // After a mixer-issued pause, Spotify Connect can briefly report the old
    // playing state for the device. Do not let that stale read clear our saved
    // paused position or re-arm the expected end time.
    if (!empty($loaded['resume_locked']) && !empty($loaded['pause_hold_until']) && time() < (int)$loaded['pause_hold_until']) {
        mx_debug_trace('spotify_playback_sync_pause_hold', [
            'deck' => $deck,
            'track_id' => (string)($loaded['id'] ?? ''),
            'pause_hold_until' => (int)$loaded['pause_hold_until'],
        ]);
        return $loaded;
    }
...
function mx_save_resume_position($deck, $device_id, $track) {
    ...
    $track['pause_hold_until'] = $now + 10;
    $track['paused_by_mixer_at'] = $now;
    unset($track['expected_finish_at']);
    mx_store_loaded_track($deck, $track);
...
function mx_pause($device_id, $deck = null) {
    mx_debug_trace('pause_start', ['deck' => $deck, 'device_id' => $device_id]);
    $device_id = trim((string)$device_id);
    if ($device_id === '') throw new RuntimeException('No Spotify device selected for this player.');
    mx_spotify_put('https://api.spotify.com/v1/me/player/pause?device_id=' . rawurlencode($device_id), '', $deck);
    mx_debug_trace('pause_end', ['deck' => $deck, 'device_id' => $device_id]);
}
...
function mx_start_loaded_deck_after_handover($deck, &$loaded, $device_id) {
    ...
    $position = !empty($loaded['played_on_deck']) ? mx_loaded_position_fallback($loaded) : 0;
    if ($position === null) $position = 0;
...
function mx_state($options = []) {
    ...
    $stateDebugSummary = [
        'device_a' => $deviceA,
        'device_b' => $deviceB,
        'loaded_a' => mx_debug_track_summary($loadedA),
        'loaded_b' => mx_debug_track_summary($loadedB),
        'playback_a' => mx_debug_playback_summary($playbackA),
        'playback_b' => mx_debug_playback_summary($playbackB),
    ];
    mx_debug_trace('state_build', $stateDebugSummary);
    return [
...
function mx_load_track_to_deck($track, $deck, $playback = null, &$playlist = null, $removeFromPlaylist = false, $loadedOrigin = '') {
    ...
    $clean = mx_reset_runtime_track_state($track);
...
function mx_return_loaded_if_unplayed($deck, &$playlist, $newTrack = null, $forceReturn = false) {
    $deck = $deck === 'b' ? 'b' : 'a';
    $loaded = mx_json('spotify_mixer_loaded_' . $deck, []);
    if (!is_array($loaded) || empty($loaded['id'])) return false;
    if (!$forceReturn && mx_track_reached_played_threshold($loaded)) return false;
    if (is_array($newTrack) && mx_track_key($loaded) === mx_track_key($newTrack)) return false;
    ...
    // Reset runtime state before putting it back, otherwise a later reload can
    // inherit stale paused/end/progress data from its previous deck run.
    $loaded = mx_reset_runtime_track_state($loaded);
...
function mx_json_out($data) {
    if (is_array($data)) {
        $data['debug'] = mx_debug_log_meta();
        mx_debug_trace('response_ready', [
            'ok' => $data['ok'] ?? null,
    ...
    mx_debug_flush((bool)($data['ok'] ?? true));
...
try {
    ...
    if ($action === 'debug_download') {
        mx_debug_download_log();
    }
    if ($action === 'debug_set') {
        $enabled = !empty($_POST['enabled']) && in_array(strtolower((string)$_POST['enabled']), ['1', 'true', 'yes', 'on'], true);
        mx_set('spotify_mixer_debug_enabled', $enabled ? '1' : '0');
        $GLOBALS['mx_debug_enabled_override'] = $enabled;
        mx_debug_trace('debug_toggle', ['enabled' => $enabled]);
        mx_json_out(['ok' => true, 'message' => 'Mixer debug mode ' . ($enabled ? 'enabled.' : 'disabled.'), 'state' => mx_state()]);
    }
    if ($action === 'debug_clear') {
        mx_debug_clear_log();
        mx_debug_trace('debug_clear', []);
        mx_json_out(['ok' => true, 'message' => 'Mixer debug log cleared.', 'state' => mx_state()]);
    }
    if ($action === 'debug_client') {
        $events = json_decode((string)($_POST['events_json'] ?? '[]'), true);
    ...
    if ($action === 'load_track_direct' || $action === 'play_track_direct') {
        ...
        if ($action === 'play_track_direct') {
            ...
            mx_play_track($device, $track['id'] ?? '', 0, $deck);
            $playedTrack = mx_json('spotify_mixer_loaded_' . $deck, []);
    ...
    if ($action === 'clear_loaded') {
        ...
        mx_pause_deck_output_for_unload($deck, $loadedForClear);
        mx_set('spotify_mixer_loaded_' . $deck, '');
        mx_set('spotify_mixer_resume_' . $deck, '');
    ...
    if ($action === 'return_loaded') {
        ...
        mx_pause_deck_output_for_unload($deck, $loadedForReturn);
        $returned = mx_return_loaded_if_unplayed($deck, $playlist, null, true);
        mx_save_playlist($playlist);
        mx_set('spotify_mixer_loaded_' . $deck, '');
        mx_set('spotify_mixer_resume_' . $deck, '');
        mx_json_out(['ok' => true, 'message' => $returned ? 'Unplayed track returned to the DJ playlist.' : 'Player ' . strtoupper($deck) . ' cleared.', 'state' => mx_state()]);
    }
    ...
    if ($action === 'play_toggle') {
        ...
        $device = $deck === 'b' ? mx_setting('spotify_mixer_device_b', '') : mx_setting('spotify_mixer_device_a', '');
        $track = mx_json('spotify_mixer_loaded_' . $deck, []);
        $transportIntent = strtolower(trim((string)($_POST['transport_intent'] ?? '')));
        if ($transportIntent !== 'play' && $transportIntent !== 'pause') $transportIntent = '';
        $postedTrackId = trim((string)($_POST['track_id'] ?? ''));
        mx_debug_trace('play_toggle_request', [
            'deck' => $deck,
            'intent' => $transportIntent,
            'posted_track_id' => $postedTrackId,
            'loaded_track_id' => (string)($track['id'] ?? ''),
            'client_tab_id' => (string)($_POST['_client_tab_id'] ?? ''),
            'ui_event_detail' => (int)($_POST['ui_event_detail'] ?? -1),
        ]);
        if ($postedTrackId !== '' && !mx_track_ids_match($postedTrackId, (string)($track['id'] ?? ''))) {
            throw new RuntimeException('The Player ' . strtoupper($deck) . ' track changed before the transport command was processed.');
        }
        if (mx_is_local_track($track)) {
            if (!empty($track['local_is_playing'])) {
    ...
    $state = null;
    try { $state = mx_state(); } catch (Throwable $stateError) { mx_debug_trace('state_after_error_failed', ['error' => $stateError->getMessage()]); }
    $payload = ['ok' => false, 'error' => $e->getMessage(), 'state' => $state, 'debug' => mx_debug_log_meta()];
    mx_debug_flush(false, ['error' => $e->getMessage()]);
