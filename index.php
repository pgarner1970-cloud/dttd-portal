...
<section class="hero">
  ...
  <?php if ($event): ?>
    <p class="subtitle"><?= h($event['event_name']) ?> at <?= h($event['venue_name']) ?><?= $event['event_date'] ? ' · ' . h(date('j F Y', strtotime($event['event_date']))) : '' ?> — follow, check in, share photos and request your favourite songs.</p>
  <?php else: ?>
    <p class="subtitle">No live event is currently available. Follow us on Facebook for upcoming events and updates.</p>
  <?php endif; ?>
</section>
...
<main class="container">
  <?php if (!$event): ?>
    <div class="card">
      ...
      <p>Song requests are only available during selected events.</p>
  ...
  <div class="grid">
    ...
    <div class="card">
      ...
      <?php if (event_requests_open($event)): ?>
        <a class="btn btn-primary" href="/request.php?event=<?= (int)$event['id'] ?>">Request a Song</a>
      <?php else: ?>
        <p class="small">Requests are now closed for this event.</p>
        <a class="btn btn-secondary" href="<?= h(FACEBOOK_URL) ?>" target="_blank">Follow Us</a>
      <?php endif; ?>
    </div>
